...
boolean IOT_BUS_SIM800::checkReply(char *reply, uint16_t timeout) {
...
void IOT_BUS_SIM800::print(char* str){
    _SIM800->println(str);
}
...
size_t IOT_BUS_SIM800::write(uint8_t c)
{
    return _SIM800->write((const uint8_t)c);
}

size_t IOT_BUS_SIM800::write(char* buffer)
{
   return _SIM800->write(buffer);
}

size_t IOT_BUS_SIM800::write(uint8_t *buffer, size_t size)
{
    return _SIM800->write(buffer, size);
}
