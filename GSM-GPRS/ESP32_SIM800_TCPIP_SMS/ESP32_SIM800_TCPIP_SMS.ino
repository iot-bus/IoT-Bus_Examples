...
char sms[32]="Test string from ESP32";

char buffer[255];
char ip[16];
char smsip[16] = "23.253.146.203";
char smsport[8] = "9999";

char port[8] = "4010";

char get[255] = "GET http://exploreembedded.com/wiki/images/1/15/Hello.txt HTTP/1.0\r\nHost: exploreembedded.com\r\n\r\n"; 
...
void loop(){
    ...
    if (SIM800.sendCheckReply("AT+CIPMUX=0", "OK", 1000) == 1)
    {
       ...
       while(SIM800.sendCheckReply2("AT+CIPSTATUS", "STATE: IP INITIAL", 1000 == 0));
        ...
        if (SIM800.sendCheckReply("AT+CSTT=\"hologram\"", "OK",   30000) == 1)
        {
            ...
            if (SIM800.sendCheckReply("AT+CIICR", "OK", 30000) == 1)
            {
                ...
                //{
                    ...
                    // Opens a TCP socket
                    sprintf(commandBuffer, "AT+CIPSTART=\"TCP\",\"%s\",\"%s\"", "exploreembedded.com", "80");
